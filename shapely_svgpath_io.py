    ...
    def from_svg_path_def(
        ...
        for key in shape_attrs:
            if not (key == "d" or key == "id"):
                path_data += ' %(key)s="%(value)s"' % {
                    "key": key,
                    "value": shape_attrs[key],
                }
...
<svg version="1.1" width="1000mm" height="1000mm" viewBox="0 0 1000 1000"
  ...
  <g id="layer">
            ...
            for subpath_poly_data in xpolys:
                if subpath_poly_data["interiors"]:
                    poly = shapely.geometry.Polygon(
                        subpath_poly_data["exterior"],
                        holes=subpath_poly_data["interiors"],
                    )
                else:
    ...
    @classmethod
    def from_shapely_linestring(
        cls, prefix: str, linestring: shapely.geometry.LineString, safe_to_close: bool
    ) -> "SvgPath":
        """ """
    ...
    def discretize_closed_path(self) -> npt.NDArray[np.complex128]:
        ...
        for k, segment in enumerate(self.svgelt_path):
            ...
            if segment.__class__.__name__ == "Line":
                if first_seg:
                    # start and end points ? no ! discretise the line
                    first_seg_is_line = True
                    _pts = [segment.point(dt * 0.1) for dt in range(10+1)]
                else:
                    # only the last point
        ...
        if first_seg_is_line:
            # first line had been discretized in 11 points
            points = np.concatenate((points[5:],points[0:5]))
        else:
