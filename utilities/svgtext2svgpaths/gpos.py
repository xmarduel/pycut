    ...
    def __init__(self, ofst: int, fontfile: FontReader):
        ...
        for i in range(scriptcnt):
            ...
            self.scripts[tag] = Script(
                tag, self.fontfile.readuint16() + scriptlisttableloc, self.fontfile
            )
        ...
        for i in range(lookupcnt):
            self.lookups.append(
                Lookup(self.fontfile.readuint16() + lookuplisttableloc, self.fontfile)
            )

    def kern(
        self, glyph1: int, glyph2: int, script: str = None, lang: str = None
    ...
    def __init__(self, tag: str, ofst: int, fontfile: FontReader):
        ...
        if self.defaultLangSysOfst:
            self.languages["DFLT"] = LanguageSystem(
                "DFLT", self.defaultLangSysOfst + self.ofst, self.fontfile
            )
        ...
        for i in range(langsyscnt):
            ...
            self.languages[tag] = LanguageSystem(
                tag, self.fontfile.readuint16() + self.ofst, self.fontfile
            )
        ...
        if self.type == 2:  # Pair adjustment positioning
            for i in range(subtablecnt):
                self.subtables.append(
                    PairAdjustmentTable(self.tableofsts[i] + self.ofst, self.fontfile)
                )
            ...
            if exttype == 2:
                self.subtables.append(PairAdjustmentTable(ptr + extofst, self.fontfile))
        else:
