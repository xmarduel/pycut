...
import gcodesimulator_webgl.gcode_syntaxhighlighter as gcode_syntaxhighlighter
from gcodesimulator_webgl.gcodeminiparser import GcodeMiniParser
...
class GCodeFileViewer(QtWidgets.QPlainTextEdit):
    ...
    def make_line_no_position_map(self):
        ...
        for line_no, line in enumerate(self.gcode.split("\n")):
            line_no_2_position[line_no + 1] = (
                1 + line_no_2_position[line_no] + len(line)
            )
    ...
    def mousePressEvent(self, event: QtGui.QMouseEvent) -> None:
        ...
        while (
            self.curr_line_no not in self.miniparser.line_no_time_map
            and self.curr_line_no > 0
        ):
            self.curr_line_no -= 1
